<section class="appUiKit-user-top-menu">
    ...
    <div class="content">
        <ul>
            <li><a href="/panel/getter?plugin=MinBlog&page=playlists" data-update="embed">Liste des playlist</a></li>
            <li><a class="active" href="/panel/getter?plugin=MinBlog&page=playlist" data-update="embed">Ajouter une playlist</a></li>
        </ul>
    </div>
</section>
<form method="post" enctype="multipart/form-data">
<section class="appkit-form-contenaire">
    <input name="submit_form" hidden value="true">
    <section class="scroll-section appUiKit-form-contenaire-color appUiKit-marginTop">
        <div class="appKitUi-input appUiKit-marginTop">
            <label>Name</label>
            <input type="text" name="name" value="<?= $getPlaylistInfo['name'] ?? '' ?>">
        </div>
    </section>
</section>
<section class="appUiKit-table">
    <table>
        ...
        <tr>
            <td><input <?php if(isset($value['checked'])) echo 'checked'; ?> id="content-input<?= $value['ID'] ?>-post" type="checkbox" name="check_post[]"></td>
            <td><b><label for="content-input<?= $value['ID'] ?>-post"><?= CMS->GFunction->htmlDecode($value['blog_title']) ?></label></b></td>
            <td><?= CMS->GFunction->web_substr(CMS->GFunction->htmlDecode($value['blog_content']), 55) ?></td>
            <td><?= CMS->GFunction->htmlDecode($value['blog_group']) ?></td>
            <td><?= CMS->GFunction->htmlDecode($value['blog_tags']) ?></td>
            <td><?= CMS->GFunction->htmlDecode($value['date_time']) ?></td>
            <td>
                <ul>
                    <li>
                        <a class="button-ui-svg" href="/panel/getter?plugin=MinBlog&page=post&ID=<?= $value['ID'] ?>" class="title-stream">
